...
app.config['MAIL_USERNAME'] = 'ilovepythonncityu@gmail.com'
app.config['MAIL_PASSWORD'] = 'Python123'
app.config['MAIL_DEFAULT_SENDER'] = 'ilovepythonncityu@gmail.com'
app.config['FLASKY_ADMIN']  = 'ilovepythonncityu@gmail.com'
mail = Mail(app)
...
def forgotpw(): # forgot password function , reply new password by email
    if 'email' in request.args:
        ...
        if len(userlogin) > 1:
            ...
            #msg = Message('Find Password', recipients=[email])
            #msg.body = ("This is the new password :", new_pass," .Please change the password now" )
            #msg.html = 'NEWPASSWORD'
            #mail.send(msg)
